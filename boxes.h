...
/**
 * @file boxes.h
 * @brief Header file for managing a graph of boxes, including functions for creating, manipulating, 
 *        and destroying the graph, as well as sorting and comparing boxes.
 */
...
typedef struct vertex {
    ...
    int index;            ///< The index of the vertex in the graph. (used for solving the Longest Path problem)
} vertex;
...
/**
 * @brief Adds all the edges to the graph, once all vertices are already created.
 *
...
/**
 * @brief Compares two boxes by their dimensions. (used as parameter for C's default qsort method)
 *
 * @param a A pointer to the first box.
 * @param b A pointer to the second box.
 ...
 *
 * This is done using C's qsort function. Note that we do not use volume as a way to check if a box fits into another,
 * as that is not proof of fitting. We sort our vertices array by their box's volume so that when finding the longest path,
 * the array is already sorted in topological order.
 *
