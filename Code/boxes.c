...
vertex *create_vertices(const char *filename, int *count) {
  ...
  *count = 1;
  ...
  for (int i = 0; i < *count; i++) {
    fscanf(file, "%hd %hd %hd", &vertices[i].b.lowest, &vertices[i].b.middle,
           &vertices[i].b.biggest);
...
bool box_fits_into(box *a, box *b) {
  return (a->lowest < b->lowest && a->middle < b->middle &&
          a->biggest < b->biggest);
}
...
int compare_boxes(const void *a, const void *b) {
  ...
  return (vertexA->b.lowest * vertexA->b.middle * vertexA->b.biggest) -
         (vertexB->b.lowest * vertexB->b.middle * vertexB->b.biggest);
}
...
void print_graph(graph *g) {
  for (int i = 0; i < g->size; i++) {
    printf("%hd %hd %hd -> ", g->vertices[i].b.lowest, g->vertices[i].b.middle,
           g->vertices[i].b.biggest);
    edge *head = g->vertices[i].head;
    while (head) {
      printf("%hd %hd %hd -> ", head->dest->b.lowest, head->dest->b.middle,
             head->dest->b.biggest);
      head = head->next;
    }
    printf("NULL\n");
...
void print_dot(graph *g) {
  ...
  for (int i = 0; i < g->size; i++) {
    printf("\t\"%p\" [label=\"%hd %hd %hd\"];\n", &g->vertices[i],
           g->vertices[i].b.lowest, g->vertices[i].b.middle,
           g->vertices[i].b.biggest);
  }
